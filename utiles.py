...
class Corpus:
    def __init__(self, args, train_data, validation_data, test_data, entity2id,
        ...
        # ratio of valid to invalid (positive to negative) samples per batch for training ConvKB Model
    ...
    def get_iteration_batch(self, iter_num):
        if (iter_num + 1) * self.batch_size <= len(self.train_indices):
            ...
            if self.invalid_valid_ratio > 0:
                ...
                # Precopying the same valid indices from 0 to batch_size to the rest
                # of the indices
            ...
            if self.invalid_valid_ratio > 0:
                ...
                # Precopying the same valid indices from 0 to batch_size to the rest of the indices
                self.batch_indices[last_index:(last_index * (self.invalid_valid_ratio + 1)), :] = np.tile(
                    self.batch_indices[:last_index, :], (self.invalid_valid_ratio, 1))
                self.batch_values[last_index:(last_index * (self.invalid_valid_ratio + 1)), :] = np.tile(
                    self.batch_values[:last_index, :], (self.invalid_valid_ratio, 1))
    ...
    def get_validation_pred(self, args, model, unique_entities):
        ...
        for iters in range(1):
            ...
            for i in range(batch_indices.shape[0]):
                ...
                # Just search for zero-th index in the sorted scores, we appended valid triple at top
            ...
            print("Current iteration Hits@100: {}".format(
                hits_at_100_head / float(len(ranks_head))))
            print("Current iteration Hits@10: {}".format(
                hits_at_ten_head / len(ranks_head)))
            print("Current iteration Hits@3: {}".format(
                hits_at_three_head / len(ranks_head)))
            print("Current iteration Hits@1: {}".format(
        ...
        print("Hits@100: {}".format(cumulative_hits_100))
        print("Hits@10: {}".format(cumulative_hits_ten))
        print("Hits@3: {}".format(cumulative_hits_three))
        print("Hits@1: {}".format(cumulative_hits_one))
